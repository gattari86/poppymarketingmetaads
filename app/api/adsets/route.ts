...
import { getAdSets, createAdSet, updateAdSetStatus } from "@/lib/meta-api";
...
export async function PATCH(request: Request) {
  ...
  try {
    const result = await updateAdSetStatus(adSetId, status, session.accessToken);
    ...
    return Response.json(
      { error: "Failed to update ad set status - Unknown error" },
      { status: 500 }
    );
  }
}
